...
![[Truth-table-XOR-gate-417x500.png]]
...
Watching the Rick and Morty episode linked in the challenge description [https://www.youtube.com/watch?v=BbcCsBj0b1U](https://www.youtube.com/watch?v=BbcCsBj0b1U) and diving into the world of Rick and Morty meme internet subculture I found [this copypasta](https://www.reddit.com/r/copypasta/comments/6qp86b/to_be_fair_you_have_to_have_a_very_high_iq_to/) which I found to be rather funny.
...
While complex, these operations begin to make sense when you realize that Wagon, while it is a stack-based concatenative language, instead of performing operations that take states to states, Wagon operations take functions from states to states, to functions from states to states as stated by its "second-order concatenative" property in the wiki page. Additionally, program states in Wagon are unbounded stacks (rather typical for stack-based languages) of unbounded integers (~~why???~~)
...
```
On the verge of switching my major to Business Development or Communications, I flip back to the description of Wagon macros and continue parsing. With the help of a Haskell expert on my team I was able to modify the last two lines of the interpreter from
```haskell
parse :: String -> Op 
parse = foldl (flip parseOp) id 
...
exec = map run . parse
```
allowing us to see the program stack at every point in its operations. With this I was able to narrow down the part of the Wagon code that was causing the problem to just the very first few characters before the @. 
...
Seeing as the language does not change capitalization freely there may be some pattern here. In fact, lowercase and uppercase characters represent functions of the same operations but with a different order where lowercase characters first perform the input function and then a set of operations while uppercase characters first perform a set of operations and then perform their input function. Thank you Wagon, very cool.
